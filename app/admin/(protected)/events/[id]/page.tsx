...
import { validateAdminId, type EventStatus, type TicketAudience, type TicketStatus } from "@/lib/admin/events";
...
export default async function EventAdminPage({ params, searchParams }: EventAdminPageProps) {
  ...
  const [{ data: event, error: eventError }, { data: tickets, error: ticketsError }] = await Promise.all([
    supabase.from("events").select("id,slug,title,description,venue,starts_at,capacity,status").eq("id", eventId).maybeSingle(),
    supabase.from("ticket_types").select("id,code,name,audience,price_cents,currency,capacity,status").eq("event_id", eventId).order("created_at", { ascending: true }),
  ]);
  ...
  const [eventPaidCount, ticketsWithCounts] = await Promise.all([
    ...
    Promise.all(typedTickets.map(async (ticket): Promise<TicketFormValue> => ({ ...ticket, paidCount: await paidTicketCount(ticket.id) }))),
  ]);
    ...
    <section>
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-semibold tracking-tight text-stone-900">{typedEvent.title}</h1>
          <p className="mt-2 text-sm text-stone-600">{eventPaidCount} paid · {typedEvent.status}</p>
        </div>
        <a className="rounded-lg border border-stone-300 bg-white px-3 py-2 text-sm font-medium text-stone-700 hover:bg-stone-50" href={`/events/${typedEvent.slug}`} target="_blank" rel="noreferrer">View public page</a>
      </div>

      <Notice error={notice.error} saved={notice.saved} />

      {eventPaidCount > 0 ? <div className="mt-6 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">Paid registrations exist. Event slug and sold ticket commercial terms are protected from destructive edits.</div> : null}
      ...
      <div className="mt-10">
        ...
        <p className="mt-1 text-sm text-stone-600">Active tickets appear on the public registration page.</p>
        <div className="mt-5 space-y-4">
          {ticketsWithCounts.map((ticket) => <TicketForm eventId={eventId} key={ticket.id} ticket={ticket} />)}
...
function Notice({ error, saved }: { error?: string; saved?: string }) {
  ...
  if (error && errors[error]) return <div className="mt-6 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{errors[error]}</div>;
  if (saved && savedMessages[saved]) return <div className="mt-6 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800">{savedMessages[saved]}</div>;
  return null;
}
